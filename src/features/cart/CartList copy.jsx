...
function CartList() {
  ...
  const orders = orderDetails?.map((orders) => orders.ordered_books);
  const booksOrders = orders?.map((books) => books.orderedBooks);
  const alreadyOrderedBookIds = booksOrders?.at(0);
  console.log(alreadyOrderedBookIds);

  // const removeFromOrdered = alreadyOrderedBookIds?.filter((bookId) => {
  //   cartList?.filter((cart) => cart.id !== bookId.id);
  // });

  // console.log(removeFromOrdered);
            ...
            >
              ...
              <div className="flex w-[20rem] flex-col items-start justify-start gap-2">
                ...
                <div className="flex flex-col items-start justify-start gap-4 text-lg font-semibold text-gray-800">
                  <p className="text-start">&#x20B9;{cart.total_book_price}</p>
                  <div
